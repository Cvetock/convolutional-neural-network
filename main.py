...
pexsels_per_image, num_labels = (748,10)
batch_size = 128
    ...
    for i in range(int(len(images) / batch_size)):
        ...
        for row_start in range(layer_0.shape[1]-kernel_rows):
            for col_start in range(layer_0.shape[2] - kernel_cols):
                sect = get_image_selection(layer_0,row_start,row_start+kernel_rows,col_start,col_start+kernel_cols)
                sects.append(sect)
        ...
        k_updete = flattened_input.T.dot(l1d_reshape)
        kernels -= alpha * k_updete
